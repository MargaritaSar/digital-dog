...
// Set constants
const float BACKGROUND_COLOR[4] = { 0.7, 0.9, 1.0, 1.0 };
const float PLANE_SIZE = 500; // Plane side width
const float PART_LIFE_SPAN = 10; // The total life of every particle in seconds
const float PART_START_POS[3] = { 0, 200, 0 }; // The starting position of every particle
...
const float GRAVITY[3] = { 0, -400, 0 }; // The gravity vector
const float WIND[3] = { 300, 0, 0 }; // The wind vector
const float MIN_WIND_Y = 30; // The minimum level (height) the wind is applied
const float MAX_WIND_Y = 80; // The maximum level (height) the wind is applied
...
void generateParticles()
{
	...
	particles.push_back(p);//creates the vector of particles
}
...
void displayParticles(void)
{
	...
	for (auto it = particles.begin(); it < particles.end(); ++it)
	{
		...
		float blue = 255 * (p.expire_time - time) / PART_LIFE_SPAN;
		glColor3ub(10, 60, blue);
		glLoadIdentity();
